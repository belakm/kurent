...
function playerMovement(){
    ...
    else if (cursors.right.isDown || gamePadButtons.right || mobileInputs.right)
    {
    ...
    if ((cursors.up.isDown || gamePadButtons.B || mobileInputs.jump) && game.time.now > jumpTimer && playerReleasedKey && player.touching){
        player.body.moveUp(520);
        jumpTimer = game.time.now + 750;
        playerReleasedKey = false;
        player.animations.play('jump');
    } else if ((cursors.up.isDown || gamePadButtons.B ||mobileInputs.jump) && !player.touching){
    ...
    if (!(cursors.up.isDown || gamePadButtons.B || mobileInputs.jump)) {
        playerReleasedKey = true;
    }
    
    if (jumpButton.isDown || gamePadButtons.Y || mobileInputs.shoot)
    {
        fire();
    }
}
...
function collisionBulletIcicle(bullet){ // DETECTS BULLETS HITTING THE FLOOR
    killBullet(bullet.sprite)
    bulletHitWall.play();
}

function collisionBulletVolkodlak(bullet, arg){ // DETECTS BULLETS HITTING VOLKODLAKS
    killBullet(bullet.sprite);
    console.log(arg);
    //bulletHitWall.play();
}